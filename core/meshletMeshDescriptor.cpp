...
namespace mm {
	...
	std::vector<std::vector<uint32_t>> KmeansS(std::vector<Triangle*>* vertices, SparseMatrix* distanceMatrix, std::vector<std::vector<uint32_t>> centers, const Vertex* vertexBuffer, NVMeshlet::GenStrategy meshletBuildingStrategy, uint32_t primitiveLimit, uint32_t vertexLimit)
	{
		...
		bool SMOOTH_CLUSTERS = meshletBuildingStrategy == NVMeshlet::KMEANSE || meshletBuildingStrategy == NVMeshlet::KMEANSEO || meshletBuildingStrategy == NVMeshlet::KMEANSA || meshletBuildingStrategy == NVMeshlet::KMEANSU;
		bool MULTI_SPLIT = meshletBuildingStrategy == NVMeshlet::KMEANSO || meshletBuildingStrategy == NVMeshlet::KMEANSEO || meshletBuildingStrategy == NVMeshlet::KMEANSA || meshletBuildingStrategy == NVMeshlet::KMEANSU;
		bool AGGRESSIVE_BALANCING = meshletBuildingStrategy == NVMeshlet::KMEANSA;

		uint32_t convergenceDist;
		uint32_t CONVERGENCE_LIM = 3 * (meshletBuildingStrategy == NVMeshlet::KMEANSU);//(*vertices).size()/1000000; // Seems to perform well?
				...
				for (uint32_t i = 0; i < clusters.size(); ++i) {
					...
					for (unsigned int j = 0; j < clusters[i].size(); ++j) {
						...
						for (Eigen::SparseMatrix<uint32_t, Eigen::RowMajor>::InnerIterator it(distanceMatrix->m_data, clusters[i][j]); it; ++it) {
							if ((*vertices)[it.col()]->flag != i || (*vertices)[it.col()]->dist != 0) continue;
							count++;
							distance = it.value();
				...
				for (uint32_t vert_id : flippableVerts) {
					...
					if (vertex->flag != old_flag) {
						for (uint32_t i = 0; i < clusters[old_flag].size(); ++i) {
							if ((*vertices)[clusters[old_flag][i]]->id == vertex->id) {
...
#pragma omp for reduction (+:differenceBetweenCenters) collapse(2) //shared(triangles, centers, clusters, CENTER_IS_SET) private(maxDistance, dist, minDistance) 
						for (int i = 0; i < clusters.size(); ++i) {
							...
							// must be further away (I THINK)
								...
								//		}
										...
										// I have no idea what is happening here ? so I will try to comment it out
		...
		case 2:
		{
			...
			std::unordered_set<uint32_t> c_indices;
			c_indices.reserve(indexVertexMap.size() / 3 / primitiveLimit);

			std::default_random_engine generator;
			std::uniform_int_distribution<uint32_t> distribution(0, triangles.size() - 1);

			while (c_indices.size() < indexVertexMap.size() / 3 / primitiveLimit) { // Consider dropping std::rand - fails to generate sufficiently random numbers
				c_indices.insert(distribution(generator)); // Can loop forever if random produces few distinct random values
			}
			...
			distanceMatrix = new  SparseMatrix(&triangles, primitiveLimit);
			clusters = KmeansS<VertexIndexType>(&triangles, (SparseMatrix*)distanceMatrix, centers, vertexBuffer, NVMeshlet::GenStrategy::KMEANSU, primitiveLimit, vertexLimit);
	...
	void createMeshletMeshDescriptors(const std::string& modelPath, std::vector<mm::MeshletGeometry>* meshletGeometry, std::vector<uint32_t>* vertCount, std::vector<Vertex>* vertices, std::vector<ObjectData>* objectData, std::vector<NVMeshlet::Stats>* stats, const int strat) {
		...
		std::vector<mm::MeshletGeometry> meshdescriptors = convertToMeshMeshlet(&vertices_model, &indices, stats, strat);

		meshletGeometry->insert(meshletGeometry->end(), meshdescriptors.begin(), meshdescriptors.end());
	...
	void createMeshletPackDescriptors(const std::string& modelPath, std::vector<NVMeshlet::MeshletGeometryPack>* meshletGeometry, std::vector<uint32_t>* vertCount, std::vector<Vertex>* vertices, std::vector<ObjectData>* objectData, std::vector<NVMeshlet::Stats>* stats, const int strat) {
		...
		NVMeshlet::MeshletGeometryPack meshdescriptors = convertToPackMeshlet(&vertices_model, &indices, stats, strat);

		meshletGeometry->push_back(meshdescriptors);
		vertices->insert(vertices->end(), vertices_model.begin(), vertices_model.end());
